...
list.of.packages <- c("raster","tidyverse","sp","virtualspecies","ggplot2","terra","sf","fuzzySim","rasterVis","viridis","colorist","rnaturalearth", "caret")
...
WorldclimS<-stack(Worldclim$bio1,Worldclim$bio4,Worldclim$bio12,Worldclim$bio15)
...
set.seed(999)
random.sp2 <- generateSpFromFun(WorldclimS,parameters = my.parameters2,formula ="2*bio1 + 4*bio4 + bio12 + 0.7*bio15")
plotResponse(random.sp2)

set.seed(999)
random.sp3 <- generateSpFromFun(WorldclimS[[c("bio1", "bio15")]],parameters = my.parameters3, formula ="2*bio1 + bio15")
plotResponse(random.sp3)

set.seed(999)
random.sp4 <- generateSpFromFun(WorldclimS[[c("bio1", "bio12")]],parameters = my.parameters4)
plotResponse(random.sp4)

### convert in PA environmental suitability
set.seed(999)
...
new.pres3<-convertToPA(random.sp3, 
                       ...
                       alpha = -0.05, plot = FALSE, 
                       species.prevalence = 0.2) 
#writeRaster(new.pres3$pa.raster,"pa_sp3.tif")
...
## sample occurrences weighted by prevalence value
set.seed(999)
...
values <- raster::extract(predictors,PA, df=T)
values <- values[,-1]
## collinearity test
Vars_to_remove <- data.frame(BIO=findCorrelation(cor(values), cutoff = .6, names = T))
...
FavModel<-multGLM(modSpecies, sp.cols = 1, var.cols=2:ncol(modSpecies), family = "binomial",
...
FavPred<- getPreds(preds, models=FavModel$models, id.col = NULL, Y = FALSE, P = TRUE,
                    Favourability = TRUE)

PredDataFav=data.frame("x" = XY$x, "y"=XY$y, "fav" = FavPred$pres_F)
...
map_multiples(metrics, palette, ncol = 2, labels = c("Virtual Species 1", "Virtual Species 2", "Virtual Species 3", "Virtual Species 4"))
#Metrics distill----
...
p2 <- map_single(metricsdist,palette)+
  ...
  ggtitle("Community of Virtual Species Distributions") + theme(plot.title = element_text(hjust = 0.5),plot.title.position ='plot')

p1 <- map_multiples(metrics, palette, ncol = 2, labels = c("Virtual Species 1", "Virtual Species 2", "Virtual Species 3", "Virtual Species 4"))+geom_sf(data=world, colour = "black", fill = "transparent") 
